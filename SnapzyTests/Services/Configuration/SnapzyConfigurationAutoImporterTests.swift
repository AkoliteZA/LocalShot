//
//  SnapzyConfigurationAutoImporterTests.swift
//  SnapzyTests
//
...
@MainActor
final class SnapzyConfigurationAutoImporterTests: XCTestCase {
  private var tempDirectory: URL!

  override func setUpWithError() throws {
    try super.setUpWithError()
    tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("SnapzyConfigurationAutoImporterTests-\(UUID().uuidString)", isDirectory: true)
  ...
  func testAutoImportAppliesChangedConfigFileAndStoresSignature() throws {
    ...
    let result = SnapzyConfigurationAutoImporter.applyIfNeeded(from: fileURL, defaults: defaults)
    ...
    let secondResult = SnapzyConfigurationAutoImporter.applyIfNeeded(from: fileURL, defaults: defaults)
